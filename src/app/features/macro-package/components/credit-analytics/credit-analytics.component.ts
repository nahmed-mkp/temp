...
@Component({
    selector: 'app-credit-analytics',
    templateUrl: './credit-analytics.component.html',
    styleUrls: ['./credit-analytics.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class CreditAnalyticsComponent implements OnInit, OnChanges {
    ...
    public customGridOption: GridOptions = {
        ...
        onCellDoubleClicked: params => {
            ...
            this.dialog.open(MacroPackageTimeseriesDialogLayoutComponent, {
                ...
                data: {
                    type: 'credit',
                    displayPropety: params.data['Ticker']
                }
            });
        },
    ...
    createDynamicColumns(analytics: any) {
        const columnNames = Object.keys(analytics[0]).filter(name => name !== 'Date' && name !== 'Index' && name !== 'Attribute'  && name !== 'Close');

        const staticColumnDefs: ColDef[] = [
            // {headerName: 'Date', field: 'Date', pinned: 'left', filter: 'agTextColumnFilter'},
            {headerName: 'Index', field: 'Index', pinned: 'left', filter: 'agTextColumnFilter', rowGroup: true, hide: true},
        ...
        columnNames.forEach(name => {
            dynamicColumnDefs.push({
                headerName: name,
                field: name,
                hide: name.includes('Percentile') || name.includes('Ticker'),

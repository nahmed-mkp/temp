...
@Component({
    selector: 'app-inflation-analytics',
    templateUrl: './inflation-analytics.component.html',
    styleUrls: ['./inflation-analytics.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class InflationAnalyticsComponent implements OnInit, OnChanges {
    ...
    createDynamicColumns(analytics: any) {
        const columnNames = Object.keys(analytics[0]).filter(name => name !== 'Date' && name !== 'Ticker'  && name !== 'Close');

        const staticColumnDefs: ColDef[] = [
            // {headerName: 'Date', field: 'Date', pinned: 'left', filter: 'agTextColumnFilter'},
            {headerName: 'Ticker', field: 'Ticker', pinned: 'left', filter: 'agTextColumnFilter', cellRenderer: params => {
    ...
    openTimeseriesPreviewTooltip(params) {
        ...
        if (params.colDef.headerName !== 'Ticker') {
            return ;
        }
